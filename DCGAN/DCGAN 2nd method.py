...
def get_data(dataroot):
    x = pd.read_csv(dataroot, usecols = ['robot_x'])
    y = pd.read_csv(dataroot, usecols = ['robot_y'])
    theta = pd.read_csv(dataroot, usecols = ['robot_theta'])
    x = x.values
    y = y.values
    theta = theta.values
...
def print_lines(array) :
    for i in range(10):
        canvas.create_line(array[0][i][0],100*sf-(array[1][i][0]),array[0][i][0]+3*sf*math.cos(array[2][i][0]),100*sf-(array[1][i][0]+3*sf*math.sin(array[2][i][0])),fill='blue',width=3,arrow='last')


def print_positions(array):
    for i in range(10):
        canvas.create_oval(array[0][i][0]-1.5*sf,100*sf-(array[1][i][0]+1.5*sf),array[0][i][0]+1.5*sf,100*sf-(array[1][i][0]-1.5*sf),fill='grey')
        canvas.create_text(array[0][i][0],100*sf-array[1][i][0],text=str(i+1))
