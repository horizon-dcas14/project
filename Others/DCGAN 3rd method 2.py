...
class Generator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.ConvTranspose2d( nz, ngf * 4, (6,2), 1, 0, bias=False),
            nn.BatchNorm2d(ngf * 4),
            nn.ReLU(True),
            # state size. (ngf*4) x 6 x 2
            nn.ConvTranspose2d(ngf * 4, ngf * 2, (3,2), 1, 0, bias=False),
            nn.BatchNorm2d(ngf * 2),
            nn.ReLU(True),
            # state size. (ngf*2) x 8 x 3
            nn.ConvTranspose2d( ngf * 2, nc, (3,1), 1, 0, bias=False),
            nn.Tanh()
...
class Discriminator(nn.Module):
    def __init__(self, ngpu):
        ...
        self.main = nn.Sequential(
            ...
            nn.Conv2d(nc, ndf, (3,1), 1, 0, bias=False),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf) x 8 x 3
            nn.Conv2d(ndf, ndf * 2, (3,2), 1, 0, bias=False),
            nn.BatchNorm2d(ndf * 2),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (ndf*2) x 6 x 2
            nn.Conv2d(ndf * 2, 1, (6,2), 1, 0, bias=False),
            nn.Sigmoid()
        )
    def forward(self, input):
            return self.main(input)
...
def clavier(event):    
    ...
    if touche == "Left":          
        ...
        print_trees([])
        print_positions(fake2[extract.get()])
        print_lines(fake2[extract.get()])                  
    elif touche == "Right":         
        extract.set(extract.get()+1)  
        canvas.delete(tkinter.ALL)
        print_trees([])
...
def print_trees(array) :
    tree1 = canvas.create_oval(tdmu(4.55076)-2*sf,100*sf-(tdmu(14.66826)+2*sf),tdmu(4.55076)+2*sf,100*sf-(tdmu(14.66826)-2*sf),fill='#000fff000')#tree_state(array[0,3]))
    tree2 = canvas.create_oval(tdmu(-0.7353)-2*sf,100*sf-(tdmu(14.75052)+2*sf),tdmu(-0.7353)+2*sf,100*sf-(tdmu(14.75052)-2*sf),fill='#000fff000')#tree_state(array[0,4]))
    tree3 = canvas.create_oval(tdmu(-15.10146)-2*sf,100*sf-(tdmu(15.76476)+2*sf),tdmu(-15.10146)+2*sf,100*sf-(tdmu(15.76476)-2*sf),fill='#000fff000')#tree_state(array[0,5]))
    tree4 = canvas.create_oval(tdmu(-2.6019)-2*sf,100*sf-(tdmu(6.7425)+2*sf),tdmu(-2.6019)+2*sf,100*sf-(tdmu(6.7425)-2*sf),fill='#000fff000')#tree_state(array[0,6]))
    tree5 = canvas.create_oval(tdmu(-1.33158)-2*sf,100*sf-(tdmu(10.02042)+2*sf),tdmu(-1.33158)+2*sf,100*sf-(tdmu(10.02042)-2*sf),fill='#000fff000')#tree_state(array[0,7]))
    tree6 = canvas.create_oval(tdmu(16.58292)-2*sf,100*sf-(tdmu(-12.5847)+2*sf),tdmu(16.58292)+2*sf,100*sf-(tdmu(-12.5847)-2*sf),fill='#000fff000')#tree_state(array[0,8]))
    tree7 = canvas.create_oval(tdmu(16.87086)-2*sf,100*sf-(tdmu(-16.01952)+2*sf),tdmu(16.87086)+2*sf,100*sf-(tdmu(-16.01952)-2*sf),fill='#000fff000')#tree_state(array[0,9]))
    tree8 = canvas.create_oval(tdmu(0.6078)-2*sf,100*sf-(tdmu(-16.23906)+2*sf),tdmu(0.6078)+2*sf,100*sf-(tdmu(-16.23906)-2*sf),fill='#000fff000')#tree_state(array[0,10]))
    tree9 = canvas.create_oval(tdmu(-16.65378)-2*sf,100*sf-(tdmu(-16.23906)+2*sf),tdmu(-16.65378)+2*sf,100*sf-(tdmu(-16.23906)-2*sf),fill='#000fff000')#tree_state(array[0,11]))
...
def print_lines(array) :
    for i in range(10):
        canvas.create_line(array[0][i][0],100*sf-(array[0][i][1]),array[0][i][0]+3*sf*math.cos(array[0][i][2]),100*sf-(array[0][i][1]+3*sf*math.sin(array[0][i][2])),fill='blue',width=3,arrow='last')


def print_positions(array):
    for i in range(10):
        canvas.create_oval(array[0][i][0]-1.5*sf,100*sf-(array[0][i][1]+1.5*sf),array[0][i][0]+1.5*sf,100*sf-(array[0][i][1]-1.5*sf),fill='grey')
        canvas.create_text(array[0][i][0],100*sf-array[0][i][1],text=str(i+1))
