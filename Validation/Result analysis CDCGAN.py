...
for i in range(len(csv_list)):
    try:
        with open('Last Epoch/CDCGAN' + str(i) + '.csv', 'r') as file:
            reader = csv.reader(file, delimiter=',')
...
cond = torch.randint(0, 2, (b_size,1,1,1))
cond = cond.float()
...
for i in range(len(fake)):
    ...
    ## write files
    if cond[i] == 0:
        with open('New results/CDCGAN_hum_' + str(i) + '.csv', 'w', newline = '') as alldata :
        ...
        with open('New results/CDCGAN_aut_' + str(i) + '.csv', 'w', newline = '') as alldata :
            writer = csv.writer(alldata,delimiter=',')
            j=0;
            for j in range(10):
                writer.writerow([data1[0][j][0],data1[0][j][1],data1[0][j][2]])
...
#        plt.plot(waterX, waterY, 'bp')
    ...
    title('Trajectorie %d' %(i))   
    if cond[i] == 0:
        plt.savefig('New results/Trajectoire_hum_%d.png' %(i))
    else: 
        plt.savefig('New results/Trajectoire_aut_%d.png' %(i))   
    show()